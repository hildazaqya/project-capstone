...
import com.example.foodection.databinding.ActivityProfileOwnerBinding
...
class ProfileOwnerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.ownerShop.setOnClickListener{
            if (shopName == null){
                startActivity(Intent(this@ProfileOwnerActivity, TambahToko::class.java))
            }else {
                startActivity(Intent(this@ProfileOwnerActivity, DetailTokoOwner::class.java))
            }
        }
    }

    private fun updateToko(userID : String, nama : String){
        db.collection("Toko").whereEqualTo("userID", userID).get().addOnCompleteListener { task ->
            if(task.isSuccessful && !task.result?.isEmpty!!){
                for(document in task.result!!){
                    val docID = document.id
                    db.collection("Toko").document(docID).update("ownerShop", nama)
                }
            }
        }
    }
